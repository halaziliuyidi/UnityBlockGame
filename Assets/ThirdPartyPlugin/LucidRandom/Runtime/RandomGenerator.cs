...
namespace AnnulusGames.LucidTools.RandomKit
{
    public class RandomGenerator
    {
        ...
        public List<T> RandomDisticncElements<T>(WeightedList<T> weightedList, int count)
        {
            if (weightedList.Count < count)
            {
                return null;
            }
        ...
        public List<T> RandomDisticncElements<T>(IEnumerable<T> values,int count)
        {
            ...
            while (result.Count<count)
            {
                T item= values.ElementAt(Range(0, count));
                if (!result.Contains(item))
                {
                    result.Add(item);
                }
